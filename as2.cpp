...
void RayTracer::trace(Ray &ray, int depth, Color* color){
	...
	for (int i = 0; i < lights.size(); i++){
		...
		if (!ag.intersectP(lray)){
			...
			r = -1 * l + 2 * (l * n) * n;
			r.normalize();
			ret = ret + (lcolor * brdf.kd * fmax(l * n, 0)) + (lcolor * brdf.ks * (pow(fmax(r * v, 0), brdf.p)));
		}
	...
	if (brdf.kr.r > 0.001 || brdf.kr.g > 0.001 || brdf.kr.b > 0.001){
        ...
        Vector k = (-1 * l + 2 * (l*n)*n);
        Point m = in.localGeo.pos;
		Ray reflectRay = Ray(&m, &k , 0, 0);
		Color tempColor;
...
int main(int argc, char *argv[]){
	if (argc == 2){
		...
		sample.y = 0;
		Ray ray;
        AggregatePrimitive m = AggregatePrimitive(list);
		RayTracer raytracer = RayTracer(m,lights, MAX_DEPTH);
		Color shading = BLACK;
		...
		while (sampler.generateSample(&sample)){
			...
			color.rgbRed = fmin(shading.r * 255, 255);
			color.rgbGreen = fmin(shading.g * 255, 255);
			color.rgbBlue = fmin(shading.b * 255, 255);
